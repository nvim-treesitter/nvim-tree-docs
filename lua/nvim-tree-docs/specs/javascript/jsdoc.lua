...
do
  ...
  local module_0_ = {config = vim.tbl_deep_extend("force", {processors = {}, slots = {}}, {empty_line_after_description = false, include_types = true, slots = {["function"] = {["function"] = true, example = false, export = true, generator = true, param = true, returns = true, template = true, yields = true}, class = {class = true, example = false, export = true, extends = true}, member = {memberof = true, type = true}, method = {example = false, generator = true, memberof = true, param = true, returns = true, yields = true}, module = {module = true}, variable = {export = true, type = true}}}), inherits = nil, lang = "javascript", module = mod_name_0_, processors = {}, spec = "jsdoc", templates = {}, utils = {}}
...
local function _4_(_241)
  ...
  return string.format(" * @returns%sThe result", type_str)
end
...
local function _6_(_241)
  return string.format(" * @function %s", _241["get-text"](_241.name))
end
...
local function _8_()
  ...
  return string.format(" * @module %s", filename)
end
...
local function _9_(_241)
  ...
  for generic in _241.iter(_241.generics) do
    ...
    table.insert(result, string.format(" * @template %s The %s type", name, name))
  end
  return result
end
...
local function _11_(_241)
  return string.format(" * @extends %s", _241["get-text"](_241.extends))
end
...
local function _13_(_241)
  return string.format(" * @class %s", _241["get-text"](_241.name))
end
...
local function _15_(_241)
  return string.format(" * @yields%s", _1_0.utils["get-marked-type"](_241, ""))
end
...
local function _17_(_241, _242)
  local description = string.format(" * The %s %s", _241["get-text"](_241.name), _242.name)
  local _18_ = _242
...
local function _18_(_241)
  local type_str = _1_0.utils["get-marked-type"](_241, " ")
  return string.format(" * @type%s", type_str)
end
...
local function _21_(_241)
  ...
  for param in _241.iter(_241.parameters) do
    ...
    table.insert(result, string.format(" * @param %s%s- The %s param", param_name, type_str, name))
  end
  return result
end
...
local function _23_(_241)
  return string.format(" * @memberof %s", _241["get-text"](_241.class))
end
...
local function _25_(_241, _242)
  return string.format(" * @%s", _242.name)
end
